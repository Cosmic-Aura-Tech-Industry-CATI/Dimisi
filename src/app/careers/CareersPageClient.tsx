    ...
    <>
      ...
      <section className="bg-[#050505] px-4 py-24 md:px-8">
        <div className="mx-auto max-w-7xl">
          <SectionHeading label="Open Positions" title="Roles We're Hiring For" subtitle="Don't see a perfect fit? Reach out anyway — we love meeting great people." />
          <div className="mt-14 flex flex-col gap-4">
            {openings.map((role, i) => {
              const isOpen = open === role.title;
              return (
                <Reveal key={role.title} delay={(i % 5) * 0.04}>
                  <div className={`rounded-2xl border transition-colors duration-300 ${isOpen ? "border-foreground/30 bg-foreground/[0.03]" : "border-foreground/10 hover:border-foreground/30 hover:bg-foreground/[0.02]"}`}>
                    <button type="button" onClick={() => setOpen(isOpen ? null : role.title)} aria-expanded={isOpen} className="flex w-full items-center justify-between gap-4 p-6 text-left">
                      <div>
                        <h3 className="text-lg font-medium text-foreground">{role.title}</h3>
                        <p className="mt-1 text-sm text-foreground/50">{role.team} · {role.type}</p>
                      </div>
                      <Plus className={`h-5 w-5 shrink-0 text-foreground/40 transition-transform duration-300 ${isOpen ? "rotate-45" : ""}`} />
                    </button>
      ...
      <section className="bg-[#050505] px-4 py-24 md:px-8">
        <div className="mx-auto max-w-7xl">
          <SectionHeading label="Recruitment Process" title="How Hiring Works" subtitle="A simple, respectful process designed to get to know each other." />
      ...
      <section className="bg-[#050505] px-4 py-24 md:px-8">
        <div className="mx-auto max-w-7xl">
          <SectionHeading label="Culture & Benefits" title="A Place to Do Your Best Work" subtitle="We invest in our people with a culture and benefits built for the long run." />
